...
torch::Tensor smooth_sampler_forward(torch::Tensor input, torch::Tensor grid, bool align_corners, bool apply_smoothstep) {
  CHECK_INPUT(input)
  CHECK_INPUT(grid)
  ...
  launch_smooth_sampler_forward_kernel(
      output, input, grid, align_corners, apply_smoothstep);
  return output;
}

std::tuple<torch::Tensor, torch::Tensor> smooth_sampler_backward(torch::Tensor grad_output, torch::Tensor input,
                                                                 torch::Tensor grid, bool align_corners, bool apply_smoothstep,
  ...
  launch_smooth_sampler_backward_kernel(
      ...
      grid, align_corners, apply_smoothstep, input_requires_grad);
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> smooth_sampler_backward_backward(torch::Tensor grad_out_input, torch::Tensor grad_out_grid,
                                                                          ...
                                                                          bool align_corners, bool apply_smoothstep) {
  ...
  launch_smooth_sampler_backward_backward_kernel(grad_input, grad_grid, grad_grad_out, input, grid,
                                                 ...
                                                 align_corners, apply_smoothstep, false);
  return std::make_tuple(grad_input, grad_grid, grad_grad_out);
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> smooth_sampler_backward_backward_grid_only(torch::Tensor grad_out_grid,
                                                                          torch::Tensor input, torch::Tensor grid, torch::Tensor grad_output,
                                                                          bool align_corners, bool apply_smoothstep) {
  CHECK_INPUT(grad_out_grid)
  CHECK_INPUT(input)
  ...
  auto grad_grad_out = torch::zeros_like(grad_output, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
  launch_smooth_sampler_backward_backward_kernel(grad_input, grad_grid, grad_grad_out, input, grid,
    torch::Tensor(), grad_out_grid, grad_output, align_corners, apply_smoothstep, true);
  return std::make_tuple(grad_input, grad_grid, grad_grad_out);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("backward_backward_grid_only", &smooth_sampler_backward_backward_grid_only, "Smooth sampler backward backward grid only (CUDA)");
}