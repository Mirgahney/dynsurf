...
bool canUse32BitIndexMath(const torch::TensorBase& t, int64_t max_elem=std::numeric_limits<int32_t>::max()) {
...
getTensorInfo(const torch::TensorBase &t) {
  IndexType sz[MAX_TENSORINFO_DIMS];
  IndexType st[MAX_TENSORINFO_DIMS];
...
__global__ void smooth_sampler_backward_kernel(
  ...
  CUDA_KERNEL_LOOP_TYPE(index, nthreads, index_t) {
    ...
    ix = grid_sampler_compute_source_index_set_grad(ix, inp_W, align_corners, &dL_dix_mult);
    iy = grid_sampler_compute_source_index_set_grad(iy, inp_H, align_corners, &dL_diy_mult);
    iz = grid_sampler_compute_source_index_set_grad(iz, inp_D, align_corners, &dL_diz_mult);
...
template <typename scalar_t, typename index_t>
static __forceinline__ __device__ void backward_backward_process_corner(
    index_t ix,
    index_t iy,
    index_t iz,
    // Slightly unprecise: in fact these are divided by surf_val.
    scalar_t dOut_dx,
    scalar_t d2Out_dx2,
    scalar_t d2Out_dxdy,
    scalar_t d2Out_dxdz,
    ...
    scalar_t d2Out_dzdx,
    scalar_t d2Out_dzdy,
    scalar_t surface_coeff,
    TensorInfo<scalar_t, index_t> grad_input,
    TensorInfo<scalar_t, index_t> grad_grad_out,
    scalar_t gOut,
    scalar_t *inp_ptr_NC,
    scalar_t *gOutGrid_ptr_NDHW,
    scalar_t *gOutInput_ptr_NC,
    scalar_t *d2L_dix2,
    scalar_t *d2L_diy2,
    scalar_t *d2L_diz2,
    index_t NC_offset_inp,
    index_t NC_offset_out,
    index_t gInp_sD,
    index_t gInp_sH,
    index_t gInp_sW,
    index_t inp_sD,
    ...
    index_t inp_H,
    index_t inp_W,
    index_t offset_out_DHW,
    const index_t grad_input_memory_span,
    const index_t grad_grad_out_memory_span
  ) {
  if (within_bounds_3d(iz, iy, ix, inp_D, inp_H, inp_W)) {
    index_t inp_el = iz * inp_sD + iy * inp_sH + ix * inp_sW;
    scalar_t surface_val = inp_ptr_NC[inp_el];
    ...
    scalar_t grad_grad_out_delta = surface_val * (dOut_dx * gOutGrid_x
                                                  + dOut_dy * gOutGrid_y
                                                  + dOut_dz * gOutGrid_z);
    ...
    fastAtomicAdd(grad_grad_out.data,
                  NC_offset_out + offset_out_DHW,
                  grad_grad_out_memory_span,
                  grad_grad_out_delta,
                  true);

    *d2L_dix2 += surface_val * gOut * (d2Out_dx2 * gOutGrid_x
                                       + d2Out_dxdy * gOutGrid_y
                                       + d2Out_dxdz * gOutGrid_z);
    *d2L_diy2 += surface_val * gOut * (d2Out_dydx * gOutGrid_x
                                       + d2Out_dy2 * gOutGrid_y
                                       + d2Out_dydz * gOutGrid_z);
    *d2L_diz2 += surface_val * gOut * (d2Out_dzdx * gOutGrid_x
...
__global__ void smooth_sampler_backward_backward_kernel(
  ...
  CUDA_KERNEL_LOOP_TYPE(index, nthreads, index_t) {
    ...
    for (int shift = 0; shift < 8; shift++) {
      ...
      out_derivatives[shift][0] = pos_corners[py][1] * pos_corners[pz][2] * pos_corners[px][3]; // dOut_dx / surf_coeff
      out_derivatives[shift][1] = pos_corners[py][1] * pos_corners[pz][2] * pos_corners[px][6]; // d2Out_dx2 / surf_coeff
      out_derivatives[shift][2] = pos_corners[py][4] * pos_corners[pz][2] * pos_corners[px][3]; // d2Out_dxdy / surf_coeff
      out_derivatives[shift][3] = pos_corners[py][1] * pos_corners[pz][5] * pos_corners[px][3]; // d2Out_dxdz / surf_coeff

      out_derivatives[shift][4] = pos_corners[px][0] * pos_corners[pz][2] * pos_corners[py][4]; // dOut_dy / surf_coeff
      out_derivatives[shift][5] = pos_corners[px][0] * pos_corners[pz][2] * pos_corners[py][7]; // d2Out_dy2 / surf_coeff
      out_derivatives[shift][6] = pos_corners[px][3] * pos_corners[pz][2] * pos_corners[py][4]; // d2Out_dydx / surf_coeff
      out_derivatives[shift][7] = pos_corners[px][0] * pos_corners[pz][5] * pos_corners[py][4]; // d2Out_dydz / surf_coeff

      out_derivatives[shift][8] = pos_corners[px][0] * pos_corners[py][1] * pos_corners[pz][5]; // dOut_dz / surf_coeff
      out_derivatives[shift][9] = pos_corners[px][0] * pos_corners[py][1] * pos_corners[pz][8]; // d2Out_dz2 / surf_coeff
      out_derivatives[shift][10] = pos_corners[px][3] * pos_corners[py][1] * pos_corners[pz][5]; // d2Out_dzdx / surf_coeff
      out_derivatives[shift][11] = pos_corners[px][0] * pos_corners[py][4] * pos_corners[pz][5]; // d2Out_dzdy / surf_coeff
    }
    ...
    for (index_t c = 0; c < C; ++c, gOut_ptr_NCDHW += gOut_sC, inp_ptr_NC += inp_sC, gOutInput_ptr_NC += gOutInput_sC, NC_offset_inp += gInp_sC, NC_offset_out += gOut_sC) {
      ...
      for (int shift = 0; shift < 8; shift++) {
        ...
        int py = (shift >> 1) & 1;
        int pz = (shift >> 2) & 1;
        backward_backward_process_corner(index_corners[px][0], index_corners[py][1], index_corners[pz][2],
                                         out_derivatives[shift][0],
                                         out_derivatives[shift][1],
                                         out_derivatives[shift][2],
                                         out_derivatives[shift][3],
                                         out_derivatives[shift][4],
                                         out_derivatives[shift][5],
                                         out_derivatives[shift][6],
                                         out_derivatives[shift][7],
                                         out_derivatives[shift][8],
                                         out_derivatives[shift][9],
                                         out_derivatives[shift][10],
                                         out_derivatives[shift][11],
                                         surface_coefficients[shift],
                                         grad_input, grad_grad_out, gOut, inp_ptr_NC, gOutGrid_ptr_NDHW,
                                         gOutInput_ptr_NC, &d2L_dix2, &d2L_diy2, &d2L_diz2, NC_offset_inp, NC_offset_out,
                                         gInp_sD, gInp_sH, gInp_sW, inp_sD, inp_sH, inp_sW,
                                         inp_D, inp_H, inp_W, offset_out_DHW,
                                         grad_input_memory_span, grad_grad_out_memory_span);
      }
    }
...
void launch_smooth_sampler_forward_kernel(
    const torch::TensorBase &output, const torch::TensorBase &input, const torch::TensorBase &grid, bool align_corners, bool apply_smoothstep) {
  auto N = input.size(0);
  auto D = grid.size(1);
  auto H = grid.size(2);
  auto W = grid.size(3);
...
void launch_smooth_sampler_backward_kernel(
    ...
    const torch::TensorBase& grid, bool align_corners, bool apply_smoothstep, bool input_requires_grad) {
  auto N = input.size(0);
  auto D = grid.size(1);
  auto H = grid.size(2);
  auto W = grid.size(3);
  ...
  if (count > 0) {
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "smooth_sampler_backward_backward_cuda", [&] {
      if (canUse32BitIndexMath(input) && canUse32BitIndexMath(grid) && canUse32BitIndexMath(grad_output)
          && canUse32BitIndexMath(grad_out_input) && canUse32BitIndexMath(grad_out_grid)) {
        smooth_sampler_backward_backward_kernel<scalar_t>
          <<<GET_BLOCKS(count, 256), 256, 0, at::cuda::getCurrentCUDAStream()>>>(
            ...
            getTensorInfo<scalar_t, int>(input),
            getTensorInfo<scalar_t, int>(grid),
            no_grad_out_input ? TensorInfo<scalar_t, int>() : getTensorInfo<scalar_t, int>(grad_out_input),
        ...
        smooth_sampler_backward_backward_kernel<scalar_t>
          <<<GET_BLOCKS(count, 256), 256, 0, at::cuda::getCurrentCUDAStream()>>>(
            ...
            getTensorInfo<scalar_t, int64_t>(input),
            getTensorInfo<scalar_t, int64_t>(grid),
            no_grad_out_input ? TensorInfo<scalar_t, int64_t>() : getTensorInfo<scalar_t, int64_t>(grad_out_input),
