...
def permutation_matrix(n):
    ...
    I, J = range(n), list(chain(range(0, n, 2), range(1, n, 2)))[::-1]
...
class RNVPDeformNetwork(nn.Module):
    def __init__(self,
                 d_feature,
                 d_in,
                 d_out_1,
                 ...
                 multires=0,
                 weight_norm=True):
        super(RNVPDeformNetwork, self).__init__()
...
class PRNVPDeformNetwork(nn.Module):
    def __init__(self,
                 d_feature,
                 d_in,
                 d_out_1,
                 ...
                 multires=0,
                 weight_norm=True):
        super(PRNVPDeformNetwork, self).__init__()
        ...
        self.rshift_tensor = torch.from_numpy(permutation_matrix(3).toarray()).float().to('cuda')
        self.lshift_tensor = self.rshift_tensor @ self.rshift_tensor
    ...
    def forward(self, deformation_code, input_pts, alpha_ratio, log_quan=None):
        ...
        for i_b in range(self.n_blocks):
            ...
            #x = torch.matmul(x, self.rshift_tensor)
            ...
            x = torch.matmul(x, self.lshift_tensor)
...
class SRNVPDeformNetwork(nn.Module):
    def __init__(self,
                 d_feature,
                 d_in,
                 d_out_1,
                 ...
                 multires=0,
                 weight_norm=True):
        super(SRNVPDeformNetwork, self).__init__()
...
class TinyAppearanceNetwork(nn.Module):
    def __init__(self,
                 d_global_feature,
                 mode,
                 d_in,
        ...
        self.mode = mode
        self.squeeze_out = squeeze_out
        n_input_dims = d_in + d_global_feature

        self.embedview_fn = None
        if multires_view > 0:
            embedview_fn, input_ch = get_embedder(multires_view)
            self.embedview_fn = embedview_fn
            n_input_dims += (input_ch - 3)

        self.tiny_net = None #tcnn.NetworkWithInputEncoding(n_input_dims=n_input_dims, n_output_dims=d_out,
                        #                              encoding_config=tiny_config["encoding"],
                        #                              network_config=tiny_config["tiny_network"])
...
class TinyGeomeryNetwork(nn.Module):
    def __init__(self,
                 d_in_1,
                 d_in_2,
                 d_out,
        ...
        if multires > 0:
            ...
            self.embedpts_fn = embedview_fn
            n_input_dims += (input_ch - 3)

        #self.tiny_net = tcnn.Network(n_input_dims, d_out, tiny_config["tiny_network"])
        n_neuron = tiny_config["tiny_network"]['n_neurons']
        self.tiny_net = nn.Sequential(
            nn.Linear(n_input_dims, n_neuron),
            nn.ReLU(),
            nn.Linear(n_neuron, d_out)
        )

    def forward(self, global_feature, points, feature_vectors):
        #global_feature = global_feature.repeat(points.shape[0], 1)
