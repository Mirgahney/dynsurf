...
"""
mask_files = []
root_dir = './mask_processed'
for file_path in tqdm(mask_files):
    file_name = file_path[-9:] #-8 
    mask_array = mask_read(file_path)
    cv2.imwrite(f"{root_dir}/{file_name}", 255 * mask_array.astype(np.uint8))

for file_path in tqdm(mask_files):
    target_file_name = f'{str(int(file_path[-9:][:-4]) + 50).zfill(5)}.png'
    dist_file = f'../mask/{target_file_name}'
    shutil.copyfile(file_path, dist_file)

for i, file_path in tqdm(enumerate(mask_files)):
    target_file_name = f'{str(i).zfill(6)}.jpg'
    dist_file = f'../rgb_/{target_file_name}'
    shutil.copyfile(file_path, dist_file)

for i, file_path in tqdm(enumerate(mask_files)):
    target_file_name = f'{str(i).zfill(6)}.png'
    dist_file = f'./depth_/{target_file_name}'
    shutil.copyfile(file_path, dist_file)
"""
"""
kfusion_frog2_grid_geo_init_bijective_False_visible_False_our_losses_trunc0.5_jade_bfspsi
"""
"""read depth image
"""
